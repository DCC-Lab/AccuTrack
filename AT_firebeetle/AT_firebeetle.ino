...
const uint8_t SROM_BYTES[] PROGMEM = {
  0x03,
  0x01,
  0x81,
  0x82,
...
BLECharacteristic ATCharacteristics("cba1d466-344c-4be3-ab3f-189f80dd7518", BLECharacteristic::PROPERTY_NOTIFY);
BLEDescriptor ATDescriptor(BLEUUID((uint16_t)0x2902));
...
const int interruptPin = GPIO_NUM_5;  // firebeetle D8;   // 5 arduino
const int nrstPin = GPIO_NUM_13;      // firebeetle D9;        // 2 arduino 
const int chipSelect = GPIO_NUM_10;    // firebeetle D6;     // 10 arduino
const int mosi = GPIO_NUM_23;
const int miso = GPIO_NUM_19;
const int led_pin = GPIO_NUM_9;
...
signed long position_X = 0;
signed long position_Y = 0;
float position_Total = 0.0;
float dist_X = 0.0;
float dist_Y = 0.0;
uint16_t squal = 0;

const float cpi = 0.001402 * 1.022808712 ; // en mm/count
...
void setup() {
  // if (bootCount == 0){
    // ++bootCount;
    gpio_hold_dis((gpio_num_t)miso);
    gpio_hold_dis((gpio_num_t)mosi);
    gpio_hold_dis((gpio_num_t)chipSelect);
    gpio_hold_dis((gpio_num_t)nrstPin);
    gpio_hold_dis((gpio_num_t)interruptPin);
    ...
    bmeService->addCharacteristic(&ATCharacteristics);
    ATDescriptor.setValue("AT value");
    ATCharacteristics.addDescriptor(&ATDescriptor);
    bmeService->start();
...
void loop() {
    ...
    if (moving){
      position_X += (signed long)readDeltaX();
      position_Y += (signed long)readDeltaY();
      position_Total = position_X*cpi;
      Serial.println(position_Total);
      sendBLE(position_Total);
    }
...
void sendDatas(float datas){
  // Serial.write((byte*)&datas, sizeof(datas));
  Serial.write((byte*)&datas, sizeof(float));
}

void goToSleep(){
  position_Total = 999999.00;
  static char posArray[6];
  dtostrf(position_Total, 6, 2, posArray);
  ATCharacteristics.setValue(posArray);
  ATCharacteristics.notify();
  Serial.println("Going to sleep...");
  delay(10);
  BLEDevice::deinit(true);
  writeRegister(SHUTDOWN, 0xB6); //Sending chip to sleep
  delay(1);
  pinMode(miso, OUTPUT);
  pinMode(mosi, OUTPUT);
  digitalWrite(miso, HIGH);
  digitalWrite(mosi, LOW);
  gpio_hold_en((gpio_num_t)miso);
  gpio_hold_en((gpio_num_t)mosi);
  gpio_hold_en((gpio_num_t)chipSelect);
  gpio_hold_en((gpio_num_t)nrstPin);
  gpio_hold_en((gpio_num_t)interruptPin);
  gpio_deep_sleep_hold_en();
  delay(500);
  esp_deep_sleep_start();
}

void sendBLE(float value){
  static char posArray[6];
  dtostrf(value, 6, 2, posArray);
  ATCharacteristics.setValue(posArray);
  ATCharacteristics.notify();
}
